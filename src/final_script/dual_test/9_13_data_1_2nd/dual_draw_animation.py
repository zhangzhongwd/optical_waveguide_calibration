...
def update(i):
    ...
    plt.plot(force[0],loss1[i],color='blue',label='1st fiber changed state')
    plt.plot(force[0], loss2[i],color='black',label='2nd fiber changed state')
    plt.xlabel('Force(N)')
    plt.ylabel('Power loss(dB)')
    plt.legend(loc='upper left')
    plt.title("Force-Power loss relationship")
    plt.text(10,5,'distance is %.2f mm'%(i*2.0))
    # return line, ax
...
if __name__ == '__main__':
    ...
    for i in range(plot_number):
        print i
        myfile = open('/home/zhong/Sensor/src/final_script/dual_test/9_13_data_1_2nd/' + (i * 1000).__str__() + '.txt')
        force_z = []
        voltage_1 = []
        voltage_2 = []
        for i in range(200):
        ...
        voltage_1 = [i-90.0 for i in voltage_1]
        voltage_2 = [i-60.0 for i in voltage_2]
    ...
    anim = FuncAnimation(fig, update, frames=11, interval=500)
