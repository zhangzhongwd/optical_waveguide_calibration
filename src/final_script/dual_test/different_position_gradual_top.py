...
if __name__ == '__main__':
    for i in range(7):
        myfile = myfile = open('/home/zhong/Sensor/src/final_script/dual_test/7_7_data_2_4st/' + ((i+6) * 1000).__str__() + '.txt')
        force_z = []
        voltage_1 = []
        voltage_2 = []
        for i in range(200):
    ...
    for j in range(7):
        plt.plot(force[0],loss1[j],color=color[j],label=label[j],linewidth=1)
    plt.xlabel('Force(N)')
